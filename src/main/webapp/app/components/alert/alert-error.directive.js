(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('commerzbankApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-commerzbankApp-error');
                var entityKey = httpResponse.headers('X-commerzbankApp-params');
                if (errorHeader) {
